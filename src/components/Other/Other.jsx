...
const Other = () => {
  ...
  if (isLoading) {
    return (
      <div
        className="blog-container section-center"
    ...
    <div className="section-center blog-container">
      {data?.map((a, key) => (
        <div key={key} className="single-blog">
          <div>
            <img
              src={a.photo_url || noImg}
              alt={a.title.slice(0, 100)}
              className="blog-img"
            />
          </div>
          <div className="blog-info">
            <h4 className="title date">
              {moment(a.published_datetime_utc).format("MMM Do YYYY")}
            </h4>
            <h4 className="title ">{a.title.slice(0, 50)}</h4>
            <a href={a.source_url} className="source-link">
              {a.source_url}
            </a>
